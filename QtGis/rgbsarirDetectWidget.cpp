...
#include "qicon.h"
rgbsarirDetectWidget::rgbsarirDetectWidget(QWidget* parent)
    : QDialog(parent)
{
    QMetaObject::Connection conn;

    setWindowTitle(QStringLiteral("光学、SAR与红外融合的伪装目标检测"));
    ...
    input_ir_button = new QPushButton;
    input_ir_button->setObjectName(QStringLiteral("input_base_button"));
    input_ir_button->setText(QStringLiteral("导入红外影像"));

    input_button_vlayout->addWidget(input_rgb_button);
    input_button_vlayout->addWidget(input_sar_button);
    input_button_vlayout->addWidget(input_ir_button);
    ...
    input_irimgedit = new QTextEdit;
    input_irimgedit->setFixedHeight(30);
    // //设置文本编辑框的尺寸策略，使其高度不随窗口拉伸而变化
    input_irimgedit->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Fixed);

    input_linedit_vlayout->addWidget(input_rgbimgedit);
    input_linedit_vlayout->addWidget(input_sarimgedit);
    input_linedit_vlayout->addWidget(input_irimgedit);
