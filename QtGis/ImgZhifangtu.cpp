...
ImgZhifangtuWidget::ImgZhifangtuWidget(QWidget* parent) : QWidget(parent)
{
    setWindowTitle(QStringLiteral("影像直方图均衡"));
    ...
    MethodComboBox->addItems({ QStringLiteral("标准直方图均衡化"), QStringLiteral("对比度受限的自适应直方图均衡化")});

    QVBoxLayout* advanced_params_vlayout = new QVBoxLayout;
    advanced_params_vlayout->addWidget(MethodComboBox);

    QGroupBox* advanced_params_box = new QGroupBox(QStringLiteral("直方图均衡化方法"));
    ...
    clip_hlayout->addWidget(new QLabel(QStringLiteral("截止限制: ")));
    clip_hlayout->addWidget(clipEdit);
    gridsize_hlayout->addWidget(new QLabel(QStringLiteral("网格大小: ")));
    ...
    QGroupBox* paramsBox = new QGroupBox(QStringLiteral("直方图均衡化参数配置"));
...
void ImgZhifangtuWidget::updateLayerComboBox(QgsLayerTreeView* treeView)
{
...
void ImgZhifangtuWidget::onLayerSelected(int index)
{
